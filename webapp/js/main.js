...
async function init() {
  ...
  try {
    ...
    setStatus("Failed to load ./data/cleaned_nbs_data.csv. Run via a local server and verify path.");
    return;
  }
...
async function loadWorldMap() {
  ...
  try {
    ...
    setStatus("Failed to load world map. Check your internet connection and try again.");
    return false;
  }
}
...
function getSelectedImpacts() {
  return getImpactCheckboxes().filter((cb) => cb.checked).map((cb) => cb.value);
}
...
function configureStartYearBounds() {
  const years = allRows.map((r) => r.start_year).filter((y) => isFiniteNumber(y));
  if (years.length === 0) {
...
function getSelectedStartYearRange() {
  if (startYearBounds.min == null || startYearBounds.max == null) return { min: null, max: null };
...
function applyFilters() {
  ...
  filteredRows = allRows.filter((r) => {
    ...
    if (selectedImpacts.size > 0) {
      ...
      const passImpacts = [...selectedImpacts].every((need) => impacts.includes(need));
...
function renderKPIs() {
  ...
  countryCountEl.textContent = String(uniqCount(filteredRows.map((d) => d.country)));
  tableMeta.textContent = `Showing ${Math.min(filteredRows.length, 250)} of ${filteredRows.length}`;
}
...
function aggregateCities(rows) {
  ...
  for (const r of rows) {
    ...
    const item = m.get(k) ?? { key: k, city: r.city ?? "", country: r.country ?? "", count: 0 };
    item.count += 1;
    m.set(k, item);
  }
  return [...m.values()];
...
function renderMap() {
  ...
  const withCoords = cityAgg
    ...
    .filter((d) => d.coords && isFiniteNumber(d.coords.lat) && isFiniteNumber(d.coords.lon));
    ...
    .join(
      (enter) =>
        enter
          ...
          .call((sel) =>
            sel.transition().duration(200).attr("r", (d) => rScale(d.count))
          ),
      (update) =>
        update
          .transition()
...
async function geocodeMissingCities(cityList) {
  ...
  try {
    ...
    if (pendingCityKeys.size > 0) {
      const cityAgg = aggregateCities(filteredRows);
      const stillMissing = cityAgg
        .filter((d) => pendingCityKeys.has(d.key) && !coordCache.has(d.key));
...
function renderCityPanel() {
  if (!selectedCity) {
    ...
    cityPanelBody.textContent = "Select a city on the map to list the interventions.";
    return;
  }
  ...
  cityPanelTitle.textContent = `${selectedCity.city}${selectedCity.country ? ", " + selectedCity.country : ""} (${rows.length})`;

  if (rows.length === 0) {
    cityPanelBody.textContent = "No results for this city under the current filters.";
    return;
  }
